class window.DBPediaInfoPlugin extends window.LimePlugin
  ...
  showAbstractInModalWindow: (annotation, outputElement) ->
    ...
    if (maintext.length >= 240)
      maintext = comment.split "."
      if (maintext[0].length >= 240)
        maintext = comment.split ","
        maintext = maintext[0] + ","
      else
        maintext = maintext[0] + "."
    ...
    result = """
             ...
             <div id="infoSecondText" style="font: Helvetica; font-family: Arial,Helvetica,sans-serif; font-size: 18px; color: #c0bebe; position: relative; float: left; background-color: #020000; vertical-align: middle; width: 300px; height: 300px; text-align: center; line-height: 1.2;">
