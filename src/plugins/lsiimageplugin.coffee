class window.LSIImagePlugin extends window.LimePlugin
  ...
  showDepictionInModalWindow: (annotation) -> # TO BE RESTRUCTURED
    ...
    lodResource = "http://new.devserver.sti2.org:8080/lsi/api/invoke?lod=" + annotation.resource.value + "&mediaType=image&limit=9&ner-radio=yes&context=" + annotation.resource.value
    ###
    ...
    console.log("LSI call: " + lodResource)
    # try
    ###
    jQuery.ajax "#{lodResource}",
      type: 'GET'
      dataType: 'xml'
    ...
    ###
    $.post "http://new.devserver.sti2.org:8080/lsi/api/invoke",
      lod: "#{annotation.resource.value}"
      mediaType: "image"
      limit: 9
      ner: "no"
      context: ""
