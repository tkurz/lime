...
(function() {
  ...
  window.LIMEPlayer = (function() {
    ...
    LIMEPlayer.prototype._initVideoPlayer = function(cb) {
	  ...
	  $("#" + this.options.containerDiv).append("<div style='font-size: 13px;'><strong>annotation</strong> - http://connectme.at/annotation/01f9e760d2fff2036b2d00f73950856c<br /><strong>fragment</strong> - https://s3-eu-west-1.amazonaws.com/08125738_standard.mp4<strong>#xywh=percent:48,55,17,15&t=1,4</strong><br /><strong>relation</strong> -  http://connectme.at/ontology#<strong>implicitlyShows</strong><br /><strong>resource</strong> - http://dbpedia.org/resource/Zorbing</div>");
      return _.defer(function() {
        ...
        return _this.player.ready(function() {
		  ...
		  _this.player.addComponent("AnnotationsSidebars");	  //add component to display 4 regions of annotations
	      _this.player.controlBar.addComponent("AnnotationToggle");		//add button to toggle annotations on/off in the control bar
		  if(!_this.player.isFullScreen) _this.player.AnnotationsSidebars.hide();
		  else _this.player.AnnotationsSidebars.show();
		  //END added SORIN
    ...
    LIMEPlayer.prototype._moveWidgets = function(isFullscreen) {
		...
		if(isFullscreen && LimePlayer.options.annotationsVisible){		//entering fullscreen, switching to 4 fixed annotation areas
			LimePlayer.widgetContainers = [{element:jQuery('.west'), orientation:'vertical'},{element:jQuery('.north'), orientation:'horizontal'},{element:jQuery('.east'), orientation:'vertical'},{element:jQuery('.south'), orientation:'horizontal'}];
    ...
    LIMEPlayer.prototype.allocateWidgetSpace = function(options) {
      ...
      container = _(this.widgetContainers).detect(function(cont) {
		//console.log(cont.element.selector);
    ...
    LIMEPlayer.prototype._hasFreeSpace = function(container, options) {
		...
		var currentHeight = container.element.height();
		var maxHeight = parseInt (container.element.css("max-height"));
		//console.log(maxHeight,currentHeight, $(window).height());
		if((!maxHeight) || (maxHeight == NaN)) maxHeight = $(window).height() - 300;
		//if(LIMEPlayer.player.isFullScreen) maxHeight = 
		//console.log(maxHeight,currentHeight);
		if(maxHeight-currentHeight < 200) return false
    ...
    TestPlugin.prototype.init = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(jQuery(annotation).bind("becomeInactive", function(e) {
		  ...
		  console.log(e.annotation.widgets);
