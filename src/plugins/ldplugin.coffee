...
class window.LDPlugin extends window.LimePlugin
  ...
  loadAnnotation: (annotation, readyCb) ->
    ...
    if annotation.isBookmark()
      ...
      # We reuse the deferred objects in order to only make one request per resource.
      # entityPromise.annotations is the array of all annotations that have the same resource linked.
      ...
      @recursiveFetch entityUri, @options.followRedirects, 2, (res) ->
        ...
        if entityUri is debug
          @lime.player.pause()
          debugger
        for annotation in annotation.entityPromise.annotations
